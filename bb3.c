...
isPromising(Vertex v) {
  printf("%d isPromising: %d\n", v.bound, v.bound > maxBene && v.weight <= totweight) ;
  return v.bound > maxBene && v.weight <= totweight ;
}
...
void
newChild(Vertex V, Item I[], int N, int W) {
  ...
  right.benefit = parent.benefit + I[right.item].b ;
  if (maxBene == 0) {
    maxBene = right.benefit ;
  }
  right.weight = parent.weight + I[right.item].w ;
  right.bound = right.benefit + Bound( &I[right.item+1], N - right.item - 1 , W - right.weight ) ;
  if (isPromising(right)) {
    insertPrm( right ) ;
    printf("insertright\n") ;
    if (right.benefit > maxBene) {
      maxBene = right.benefit ;
    }
  }
